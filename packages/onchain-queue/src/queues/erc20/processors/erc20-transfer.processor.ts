...
@Processor(ONCHAIN_QUEUES.QUEUE_TRANSFER_20)
export class ERC20TransferProcessor {
  ...
  logger = new Logger(ERC20TransferProcessor.name);

  @Process({ name: ONCHAIN_JOBS.JOB_TRANSFER_20, concurrency: 10 })
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_TRANSFER_20, event);
    }
  }
}
