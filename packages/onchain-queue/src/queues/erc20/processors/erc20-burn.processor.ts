...
@Processor(ONCHAIN_QUEUES.QUEUE_BURN_20)
export class ERC20BurnProcessor {
  constructor(
  ...
  logger = new Logger(ERC20BurnProcessor.name);

  @Process({ name: ONCHAIN_JOBS.JOB_BURN_20, concurrency: 100 })
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_BURN_20, event);
    }
  }
}
